...
def sample_student():
    return student_repository.create_student('S_SURVEY_TEST', 'Survey Test Student', 'survey.test@example.com',
                                             'MSc Survey Testing', 1)
...
def test_create_and_get_survey_response(sample_student, sample_module):
    new_response = survey_response_repository.create_survey_response(student_id=sample_student.id,
                                                                     module_id=sample_module.id, week_number=4,
                                                                     stress_level=5, hours_slept=6.5,
                                                                     mood_comment="Feeling stressed")
